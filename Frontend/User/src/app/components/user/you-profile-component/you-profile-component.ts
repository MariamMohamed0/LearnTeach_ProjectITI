...
@Component({
  ...
  imports: [CommonModule, FormsModule, HttpClientModule, ReactiveFormsModule, Sidestudent, UserHeader, RouterLink, FontAwesomeModule,ChatWindowComponent,FormsModule,MakeReportComponent],
 standalone: true,
...
//       if (chatId) {
//         // نفتح صفحة الشات مع تمرير receiverId
//         window.location.href = `/chat/${chatId}?receiverId=${user2Id}`;
//       }
//     },
//     error: err => console.error(err)
//   });
...
openChatWithUser(user2Id?: number) {
  ...
  this.http.post<{ chatId: string }>(
    ...
    next: res => {
      const chatId = res.chatId;
      if (chatId) {
        // ✅ تنقل صحيح داخل Angular بدون Reload
